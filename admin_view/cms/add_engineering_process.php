...
<div id="content">
 ...
 <div class="container-fluid">
  ...
  <a href="<?php echo base_url('admin/engineering-process');?>" data-bs-toggle="tooltip" title="Back" class="btn btn-light">
   ...
   <div id="tab-general" class="tab-pane active">
      <fieldset>
           ...
           <div class="row mb-3 required">
              <label for="input-username" class="col-sm-2 col-form-label">Select Infrastructure</label>
              <div class="col-sm-10">
                  <select name="infra_id" class="form-control">
                    <option value="">Select</option>
                    <?php foreach ($infraList as $key => $value): ?>
                      <option value="<?php echo $value->id; ?>" <?php echo $infra_id==$value->id?'selected':'';?>  ><?php echo $value->name; ?></option>
                    <?php endforeach ?>
                  </select>

                 <?php echo $validation->hasError('infra_id')?$validation->showError('infra_id','my_single'):''; ?>
              </div>
          </div>
