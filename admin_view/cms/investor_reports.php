    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="card">
          ...
          <div class="card-body">
            <form action="<?php echo base_url('admin/investor_reports') ?>" method="get">
           
            <div class="mb-3">
              <label for="input-name" class="form-label">Financial year</label>
            
              <select name="year_id" class="form-control">
                <option value="">Select</option>
                <?php foreach ($yearList as $key => $value): ?>
                       <option value="<?php echo $value->id;  ?>" <?php echo @$_GET['year_id']==$value->id?'selected':''; ?>  ><?php echo $value->name; ?></option>
                <?php endforeach ?>
              </select>
            ...
            <div class="text-end">
              ...
              <a href="<?php echo base_url('admin/investor_reports'); ?>"><button type="button" id="button-filter" class="btn btn-light"><i class="fa-solid fa-filter"></i> Reset</button></a>
            </div>
            </form>
          </div>
      ...
      <?php if ($error = session()->getFlashdata('error')): ?>
       ...
       <?php endif ?>
            ...
            <div id="user" class="card-body">
               <?php echo form_open('admin/delete_investor_reports','id="form-user"'); ?> 
                            ...
                            <thead>
                                <tr>
                                 <td style="width: 1px;" class="text-start"><input type="checkbox" onclick="$('input[name*=\'selected\']').prop('checked', this.checked);" /></td>
                                   ...
                                   <td class="text-start">Financial Year</td>
         ...
         <?php foreach ($detail as $key => $value){  ?>
           ...
           <td class="text-left"><?php echo $value->year_name; ?></td>

           <td class="text-left"><?php echo $value->status==1?'Active':'Deactive'; ?></td>

          
           <td class="text-right">
            <a href="<?php echo base_url('admin/add_investor_report/'.$value->id); ?>" data-toggle="tooltip" title="Edit" class="btn btn-primary">
