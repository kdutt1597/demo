...
use App\Models\Cms\CloudCatModel;
use App\Models\Cms\CloudMediaModel;
...
class Cloud extends BaseController
{
    ...
    public function index(){
		...
		// pagination
			$data['perPage'] = 10;
			$data['detail'] = $model->asObject()->where($query)->like($like)->orderBy('cloud_cats.id','asc')->paginate($data['perPage']);
		...
		echo view('admin/product/cloud_cats',$data);

	}

	public function add_cloud_cats($id=false){

        $model = new CloudCatModel();

        if (!empty($id)) {
            $data['page_title'] = 'Edit Cloud Cats';
            $data['form_action'] = 'admin/add_cloud_cats/' . $id;
            ...
            $data['page_title'] = 'Add Cloud Cats';
            $data['form_action'] = 'admin/add_cloud_cats';
            $data['name'] = '';
            ...
            if ($this->validate($rules) == false) {
                ...
                if ($id) {
                    ...
                    if ($result) {
                        ...
                        return redirect()->to('admin/add_cloud_cats/' . $id);
                    } else {
                    ...
                    if ($result) {
                        ...
                        return redirect()->to('admin/cloud_cats');
                    } else {
                        $this->session->setFlashdata('error', 'Record not inserted');
                        return redirect()->to('admin/add_cloud_cats');
                    }
                }
            }
        }

        echo view('admin/product/add_cloud_cats', $data);
	}
	
	public function delete_cloud_cats(){
       $model = new CloudCatModel();
    ...
    public function cloud_blogs(){
        $model = new CloudMediaModel();
        ...
        $builder = $model->asObject()
            ->select('cyb_cloud_blog.*')
            ->orderBy('cyb_cloud_blog.id', 'desc');
        ...
        $data['config_logo'] = $this->config_logo;
        return view('admin/module/cloud_blogs', $data);
    }

    public function add_cloud_blog($id = false){
		...
		$data['form_action'] = 'admin/add_cloud_blog/'.$id;	
		...
		$data['form_action'] ='admin/add_cloud_blog';
		$data['footer'] =  '';
		$data['title'] = '';
		$data['description'] = '';
		$data['status'] = '';
   	...
   	if(!empty($_FILES['whitepaper_download']['name'])){
		...
		if(!empty($id)){
			...
			if($result){
				...
				return redirect()->to('admin/add_cloud_blog/'.$id);
			}else{
			...
			if($result){
				...
				return redirect()->to('admin/cloud_blogs');
			}else{
				session()->setFlashdata('error','Record update unsuccess');
				return redirect()->to('admin/add_cloud_blog');
			}
		...
		return view('admin/module/add_cloud_blog',$data);
